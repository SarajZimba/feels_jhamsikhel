...
#         flag = True
#         order = Order.objects.filter(Q(table_no=table_no) & Q(outlet=outlet) & (Q(state='Pending') | Q(state='Cooked') | Q(state='Accepted')))
#         if order.exists():
...
class CheckOrderCode(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        existing_orders = Order.objects.filter(Q(table_no=table), Q(outlet=branch), Q(status=True), ~Q(state='Completed'), ~Q(state='Cancelled'))
        if existing_orders.exists():
            if existing_orders.filter(code=code).exists():
